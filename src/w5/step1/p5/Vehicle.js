class Vehicle {
  constructor(x, y, maxSpeed = 5, maxForce = 0.1, friendRad = 80) {
  ...
  separate(allVehicles, factor = 1) {
    ...
    allVehicles.forEach((aVehicle) => {
      if (aVehicle !== this) {
        ...
        if (dist > 0 && dist < this.friendRad / 2) {
          const diff = p5.Vector.sub(this.pos, aVehicle.pos);
          diff.normalize();
          diff.div(dist);
          sum.add(diff);
          count++;
    ...
    if (count > 0) {
      ...
      this.applyForce(steer.mult(factor));
    }
  }
